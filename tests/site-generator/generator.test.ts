...
function copyTemplateWorkspace(): string {
	...
	// Strip site-specific generated residue so fixture generate/build is deterministic.
	rmSync(path.join(dir, '.site-generator-manifest.json'), { force: true });
	rmSync(path.join(dir, 'src/config/indexnow-key.json'), { force: true });
	rmSync(path.join(dir, 'src/content/docs'), { recursive: true, force: true });
	mkdirSync(path.join(dir, 'src/content/docs'), { recursive: true });
	writeFileSync(path.join(dir, 'src/content/docs/index.mdx'), '---\ntitle: Placeholder\n---\n');
	rmSync(path.join(dir, 'src/content/trust'), { recursive: true, force: true });
	mkdirSync(path.join(dir, 'src/content/trust'), { recursive: true });
	for (const name of readdirSync(path.join(dir, 'public'))) {
		if (/^[0-9a-f-]{36}\.txt$/i.test(name)) rmSync(path.join(dir, 'public', name), { force: true });
	}
	return dir;
}
...
test('13.3b G027 sole analytics.ga4MeasurementId emits runtime config', () => {
	const workspace = copyTemplateWorkspace();
	try {
		installFixture(workspace, 'valid-site');
		installTrustInputs(workspace);
...
`,
		...
		generateSite({ specPath: path.join(workspace, 'site-spec.yaml'), rootDir: workspace });
		const generated = readFileSync(path.join(workspace, 'src/config/site.generated.ts'), 'utf8');
		assert.match(generated, /ga4MeasurementId: ["']G-TEST123456["']/);
		assert.match(generated, /ga4: \{ enabled: true \}/);
...
test('13.3c G027 sole analytics without Measurement ID omits analytics block', () => {
	const workspace = copyTemplateWorkspace();
	try {
		installFixture(workspace, 'valid-site');
		installTrustInputs(workspace);
		...
		const generated = readFileSync(path.join(workspace, 'src/config/site.generated.ts'), 'utf8');
		assert.doesNotMatch(generated, /\banalytics:\s*\{/);
		assert.doesNotMatch(generated, /ga4MeasurementId/);
	} finally {
		rmSync(workspace, { recursive: true, force: true });
	}
});
