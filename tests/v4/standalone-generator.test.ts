...
test('standalone generator emits a complete canonical V4 site into an empty destination', () => {
	...
	try {
		...
		assert.equal(existsSync(path.join(destination, 'src/lib/indexnow.ts')), true);
		assert.equal(existsSync(path.join(destination, 'scripts/indexnow-submit.ts')), true);
		assert.match(readFileSync(path.join(destination, 'package.json'), 'utf8'), /"indexnow"/);
		assert.match(readFileSync(path.join(destination, 'astro.config.mjs'), 'utf8'), /gw-ga4-runtime/);
		...
		assert.doesNotMatch(standalonePage, /V4Analytics|Analytics/);
...
test('standalone generator writes PUBLIC_GA_MEASUREMENT_ID from analytics.ga4MeasurementId', () => {
	const workspace = mkdtempSync(path.join(os.tmpdir(), 'g027-v4-ga4-'));
	try {
		const inputPath = path.join(workspace, 'input.json');
		const input = JSON.parse(readFileSync(fixture, 'utf8'));
		input.analytics = { ga4MeasurementId: 'G-TEST123456' };
		writeFileSync(inputPath, `${JSON.stringify(input)}\n`, 'utf8');
		const destination = path.join(workspace, 'site');
		generateV4Standalone(inputPath, destination);
		assert.equal(readFileSync(path.join(destination, '.env'), 'utf8').trim(), 'PUBLIC_GA_MEASUREMENT_ID=G-TEST123456');
		assert.match(readFileSync(path.join(destination, 'astro.config.mjs'), 'utf8'), /gw-ga4-runtime/);
		assert.doesNotMatch(readFileSync(path.join(destination, 'src/pages/index.astro'), 'utf8'), /V4Analytics|gtag/);
	} finally {
