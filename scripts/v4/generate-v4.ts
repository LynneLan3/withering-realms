...
export interface V4SiteInput {
	...
	/**
	 * G027 sole analytics input for V4 / site:create.
	 * Mapped to PUBLIC_GA_MEASUREMENT_ID; runtime lives in standalone astro.config.mjs.
	 */
...
export function generateV4Standalone(inputPath: string, destination: string): V4GeneratedSite {
	...
	const input = JSON.parse(readFileSync(inputPath, 'utf8')) as V4SiteInput;
	const generated = toCanonicalSite(generateV4(input));
	...
	// G027: preserve IndexNow capability on standalone package output.
	mkdirSync(path.join(destination, 'src/lib'), { recursive: true });
	if (existsSync(path.join(root, 'src/lib/indexnow.ts'))) {
		cpSync(path.join(root, 'src/lib/indexnow.ts'), path.join(destination, 'src/lib/indexnow.ts'));
	}
	mkdirSync(path.join(destination, 'scripts'), { recursive: true });
	if (existsSync(path.join(root, 'scripts/indexnow-submit.ts'))) {
		cpSync(path.join(root, 'scripts/indexnow-submit.ts'), path.join(destination, 'scripts/indexnow-submit.ts'));
	}
	...
	// G027: map analytics.ga4MeasurementId → PUBLIC_GA_MEASUREMENT_ID for config-owned runtime.
	const ga4MeasurementId = String(input.analytics?.ga4MeasurementId || '').trim();
	if (ga4MeasurementId) {
		if (!/^G-[A-Z0-9]+$/.test(ga4MeasurementId)) {
			throw new Error(`Invalid analytics.ga4MeasurementId: ${ga4MeasurementId}`);
		}
		writeFileSync(path.join(destination, '.env'), `PUBLIC_GA_MEASUREMENT_ID=${ga4MeasurementId}\n`, 'utf8');
	}
	return generated;
}
