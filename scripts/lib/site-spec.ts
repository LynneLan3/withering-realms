...
export interface SiteSpecAnalytics {
	...
	/**
	 * G027 sole template analytics input (preferred).
	 * Create/bootstrap maps this to PUBLIC_GA_MEASUREMENT_ID for runtime.
	 */
	ga4MeasurementId?: string;
	/** Legacy GA4 fields retained for existing specs; new specs must use ga4 / ga4MeasurementId. */
...
function parseAnalytics(raw: unknown, location: string): SiteSpecAnalytics | undefined {
	...
	const hasSoleMeasurementId = analyticsRecord.ga4MeasurementId !== undefined && analyticsRecord.ga4MeasurementId !== null && String(analyticsRecord.ga4MeasurementId).trim() !== '';
	// G027: analytics.ga4MeasurementId alone is a valid create-path contract.
	const hasContract = hasIdentityContract || hasProviderSwitches || hasSoleMeasurementId;
	...
	if (hasIdentityContract) {
		...
		const ga4MeasurementIdRaw = requireString(analyticsRecord, 'ga4MeasurementId', `${location}.ga4MeasurementId`, { optional: true });
		if (ga4MeasurementIdRaw && !GA4_MEASUREMENT_ID.test(ga4MeasurementIdRaw)) {
			fail('analytics.ga4MeasurementId must be a GA4 ID.', 'analytics.ga4MeasurementId', ga4MeasurementIdRaw, `${location}.ga4MeasurementId`, 'Use a GA4 measurement ID such as G-XXXXXXXXXX.');
		}
		const identityTrackOutbound = requireBoolean(analyticsRecord, 'trackOutbound', `${location}.trackOutbound`, { optional: true });
		return {
			enabled: true,
			siteId,
			gameSlug,
			templateVersion,
			launchDate,
			ga4: providerSwitch('ga4'),
			vercelAnalytics: providerSwitch('vercelAnalytics'),
			ga4MeasurementId: ga4MeasurementIdRaw || undefined,
			...(identityTrackOutbound !== undefined
				? { trackOutbound: identityTrackOutbound }
				: ga4MeasurementIdRaw
					? { trackOutbound: true }
					: {}),
		};
	...
	if (hasProviderSwitches) {
		...
		const switchTrackOutbound = requireBoolean(analyticsRecord, 'trackOutbound', `${location}.trackOutbound`, { optional: true });
		return {
			enabled: true,
			ga4: optionalProviderSwitch('ga4'),
			vercelAnalytics: optionalProviderSwitch('vercelAnalytics'),
			ga4MeasurementId: ga4MeasurementIdRaw || undefined,
			...(switchTrackOutbound !== undefined
				? { trackOutbound: switchTrackOutbound }
				: ga4MeasurementIdRaw
					? { trackOutbound: true }
					: {}),
		};
	}
	// G027 sole-input shape: analytics.ga4MeasurementId without full identity contract.
	const soleId = requireString(analyticsRecord, 'ga4MeasurementId', `${location}.ga4MeasurementId`, { optional: true });
	if (soleId) {
		if (!GA4_MEASUREMENT_ID.test(soleId)) {
			fail('analytics.ga4MeasurementId must be a GA4 ID.', 'analytics.ga4MeasurementId', soleId, `${location}.ga4MeasurementId`, 'Use a GA4 measurement ID such as G-XXXXXXXXXX.');
		}
		return {
			enabled: true,
			ga4: { enabled: true },
			ga4MeasurementId: soleId,
			trackOutbound: requireBoolean(analyticsRecord, 'trackOutbound', `${location}.trackOutbound`, { optional: true, defaultValue: true })!,
		};
	}
