...
function renderSiteGeneratedTs(config: GameConfig): string {
	...
	if (config.analytics) {
		...
		if (config.analytics.ga4MeasurementId) lines.push(`\t\tga4MeasurementId: ${tsString(config.analytics.ga4MeasurementId)},`);
...
function buildAnalyticsConfig(spec: SiteSpec): GameAnalyticsConfig | undefined {
	...
	const ga4MeasurementId = spec.analytics.ga4MeasurementId || spec.analytics.measurementId;
...
function assertNoCollision(
	...
	if (expectedManaged.has(relativePath) && existingManifest === null) {
		...
		// IndexNow key files may already exist when a prior generated site left them in the tree;
		// reuse them as managed outputs rather than colliding.
