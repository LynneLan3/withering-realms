...
## Analytics contract (G027 / V4 create path)

New sites use a single template input:
...
`site:create` / V4 standalone maps this to `PUBLIC_GA_MEASUREMENT_ID`. The GA4
runtime is owned by the standalone Astro config integration (not page UI
components), so presentation upgrades do not drop analytics. Production loads
GA4 with automatic `page_view` plus outbound link tracking; a missing ID is a
safe no-op (build succeeds, no gtag injection). Deploy env may still override
`PUBLIC_GA_MEASUREMENT_ID`. Legacy `ga4.enabled` / identity fields remain
supported when supplied.
